...
/*   Updated: 2021/03/05 15:58:23 by tnave            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int				ft_before_diux(t_pfstruct *pf, t_pfconv *c_conv, long nb, long i)
{
	...
	if (c_conv->dash != 1) 
	{
		if (c_conv->zero == 1)
			...
			ft_padding(' ', pf, c_conv->nb_width - (ft_max(i, c_conv->dot_prec))); // 12 - 3 = 9
	}
	if (pf->neg && !c_conv->zero)
		add_to_buff(pf, '-');
	if (c_conv->dot)
	...
	// 		return (0);
	// 	}
	// 	if (c_conv->dot_prec == 0 && c_conv->nb_width > 0 && c_conv->zero == 1)
	// 	{
	// 		ft_padding(' ', pf, c_conv->nb_width);
	// 		return (0);
	// 	}
	// 	if (c_conv->dot_prec == 0 && c_conv->dash == 1)
	// 	{
	// 		ft_padding(' ', pf, c_conv->nb_width);
	// 		return (1);
	// 	}
